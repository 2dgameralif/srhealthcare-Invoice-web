...
const FIRESTORE_BACKUP_COLLECTIONS = ["users", "invoices", "products", "customers", "stocks", "settings"];
    ...
    try {
        ...
        reader.onload = async (e) => {
            ...
            for (let i = 0; i < lines.length; i += 2) {
                ...
                if (name && !isNaN(price)) toImport.push({ name, price });
            }
            let count = 0;
...
function renderShopInfo(data) {
    ...
    tbody.innerHTML = data.map(c => `<tr><td>${c.billTo}</td><td>${c.shipTo || ''}</td><td>${c.phone || ''}</td>`).join('');
}

async function loadUserSalesHistory(userId, forceRefresh = false) {
    ...
    try {
        ...
        if (cached && !forceRefresh) {
            renderUserSalesHistory(JSON.parse(cached));
            return;
        }
